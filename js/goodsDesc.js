define(["jquery","jquery-cookie"],function($){
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                <div class = 'xm-buyBox' id = 'J_buyBox'>
                    ...
                    /* 判断图片有几张 */
                    if(aImages.length == 1){
                        $(`<img class = 'slider done' src="${aImages[0]}" 
                        style="float: none; list-style: none; position: absolute; 
                        width: 560px; z-index: 0; display: block;" alt=""/>`).appendTo(node.find('#J_sliderView'));
                        /* 隐藏上一张和下一张图片 */
                        ...
                        /* 创建图片本身 */
                        $(`<img class = 'slider done' src="${aImages[i]}" 
                        style="float: none; list-style: none; position: absolute; 
                        width: 560px; z-index: 0; display: ${i == 0 ? 'block' : 'none'};" alt=""/>`).appendTo(node.find('#J_sliderView'));
                        }
                    }
            },
            error:function(msg){
    ...
    function banner(){
        ...
        $('#app div').on('click','.J_login',function(){
            /* 获取点击购物车的商品id
            cookie本地存储 存储商品id和数量 
            【{id:1,num1},{id:2,num2}】转成json格式字符串*/
            var id = this.id;
            /* 判断是否是第一次添加 */
